...
3. **Update the `images` Array**:
   ...
   - **To Replace/Swap an Image**: Replace the target index or URL string with the new file path (e.g., `/uploads/new_pic.jpg` or an external `https://...` URL).
   - **If User Attached a File in Chat**: Reference the file uploaded to the project workspace (typically placed in `/public/uploads/` or `/uploads/`).
